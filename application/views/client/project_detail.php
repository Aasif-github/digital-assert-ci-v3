...
<body>
    <div class="container">
        ...
        <!-- <a href="</?php echo site_url('client'); ?>" class="btn btn-secondary btn-sm float-end"><i
                class="fa-solid fa-arrow-left"></i>&nbsp;Back</a> -->
            <div class="col-md-8">
            <h2 class="mt-5 d-flex justify-content-center align-items-center"><?php echo htmlspecialchars($project['project_name']); ?></h2>    
                <?php if ($project['project_thumbnail']): ?>
                    <img src="<?php echo base_url('public/' . $project['project_thumbnail']); ?>" class="img-fluid mb-3"
                        alt="<?php echo htmlspecialchars($project['project_name']); ?>">
                <?php endif; ?>
                <p><strong>Short Description:</strong>
                    <?php echo htmlspecialchars($project['project_short_description'] ?: 'N/A'); ?></p>
                <p><strong>Long Description:</strong>
                    <?php echo htmlspecialchars($project['project_long_description'] ?: 'N/A'); ?></p>
        ...
        <h3 class="my-4 d-flex justify-content-center align-items-center">Media Files</h3>
        <div class='col-md-8 mx-auto'>
            <!-- Search Bar -->
            <div class="mb-4">
                <input type="text" id="userInput" class="form-control"
                    placeholder="Search by media type, file title, or description..." oninput="handleInput()">
            </div>
        </div>        

        <div id="resultDiv" style="display: none;"></div>
        
        <div id="originalDiv" tabindex="0">
                    ...
                    <?php if (!empty($files)): ?>
                        <div class="accordion-item">
                            <h2 class="accordion-header" id="heading<?php echo $type; ?>">
                                <button class="accordion-button <?php echo $first_non_empty ? '' : 'collapsed'; ?>" type="button"
                                    data-bs-toggle="collapse" data-bs-target="#collapse<?php echo $type; ?>"
                                    aria-expanded="<?php echo $first_non_empty ? 'true' : 'false'; ?>"
                                    aria-controls="collapse<?php echo $type; ?>">
                                    <?php echo $type; ?> (<?php echo count($files); ?>)
                                </button>
                            </h2>
                            <div id="collapse<?php echo $type; ?>"
                                class="accordion-collapse collapse <?php echo $first_non_empty ? 'show' : ''; ?>"
                                aria-labelledby="heading<?php echo $type; ?>" data-bs-parent="#mediaAccordion">
                                            ...
                                            <div class="col-md-4 mb-4">
                                                <div class="card">
                                                    ...
                                                    <?php if (in_array($file_extension, ['jpg', 'jpeg', 'png', 'gif'])): ?>
                                                        <img src="<?php echo base_url('public/' . $media['file_url']); ?>"
                                                            class="card-img-top" alt="<?php echo htmlspecialchars($media['title']); ?>">
                                                    <?php elseif ($file_extension === 'mp3'): ?>
                                                        <div class="p-3">
                                                            <audio controls class="media-preview">
                                                                <source src="<?= base_url('public/' . $media['file_url']); ?>"
                                                                    type="<?php echo htmlspecialchars($media['mime_type'] ?: 'audio/mpeg'); ?>">
                                                    ...
                                                    <?php elseif (in_array($file_extension, ['mp4', '3gp'])): ?>
                                                        <div class="p-3">
                                                            <video controls class="media-preview">
                                                                <source src="<?= base_url('public/' . $media['file_url']); ?>"
                                                                    type="<?php echo htmlspecialchars($media['mime_type'] ?: 'video/mp4'); ?>">
                                                                Your browser does not support the video element.
                                                            </video>
                                                        </div>
                                                    <?php elseif ($file_extension === 'pdf'): ?>
                                                        <div class="p-3">
                                                            <iframe src="<?= base_url('public/' . $media['file_url']); ?>"
                                                                class="media-preview"
                                                                title="<?php echo htmlspecialchars($media['title']); ?>"></iframe>
                                                        </div>
                                                    ...
                                                    <?php elseif (in_array($file_extension, ['doc', 'docx'])): ?>
                                                        ...
                                                        <div class="card-img-top text-center p-3 bg-light">File:
                                                            <?php echo htmlspecialchars($media['title']); ?></div>
                                                    <?php else: ?>
                                                        <div class="card-img-top text-center p-3 bg-light">File:
                                                            <?php echo htmlspecialchars($media['title']); ?></div>
                                                    <?php endif; ?>
                                                    <div class="card-body">
                                                        <h5 class="card-title">
                                                            <?php echo htmlspecialchars($media['title'] ?: 'Untitled'); ?></h5>
                                                        <p class="card-text">
                                                            <?php echo htmlspecialchars($media['description'] ?: 'No description'); ?>
                                                        </p>
                                                        <p>Type: <?php echo htmlspecialchars($media['file_type']); ?> | Size:
                                                            <?php echo round( $media['file_size']); ?> KB</p>
                                                        <p>Uploaded by: <?php echo htmlspecialchars($media['username'] ?? 'Unknown'); ?>
                                                        </p>
                                                        <a href="<?php echo base_url('public/' . $media['file_url']); ?>"
    ...
    <script>
        ...
        function handleInput() {
            ...
            // when search input has zero input 
            if(input.length == 0){ 
                document.getElementById("originalDiv").style.display = "block";
            }

            const index = currentUrl.indexOf('/index.php');
            ...
            timeout = setTimeout(() => {
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        ...
                        if (data.status === 'success' && data.data.length > 0) {
                            data.data.forEach(project => {
                                const fileType = project.media.file_type.toLowerCase();
                                const fileTitle = project.media.title;
                                const newFileUrl = project.media.file_url.replace('/storage/', '/public/storage/');
                                const fileSize = project.media.file_size;
                                // console.log(fileType);
                                // Dynamically decide preview HTML
                                let mediaPreview = '';

                                if (['image'].includes(fileType)) {
                                    mediaPreview = `<img src="${newFileUrl}" class="card-img-top img-fluid" alt="${fileTitle}" style="object-fit: cover; height: 200px;">`;
                                } else if (['video'].includes(fileType)) {
                                    mediaPreview = `
                            <video controls class="card-img-top" style="object-fit: cover; height: 200px;">
                                <source src="${newFileUrl}" type="video/${fileType}">
                                Your browser does not support the video tag.
                            </video>`;
                                } else if (['audio'].includes(fileType)) {
                                    mediaPreview = `
                            <div class="p-2 text-center" style="height: 200px; background-color: #f8f9fa;">
                                <audio controls class="w-100">
                                    <source src="${newFileUrl}" type="audio/${fileType}">
                                    Your browser does not support the audio element.
                                </audio>
                            </div>`;
                                } else if (fileType === 'pdf') {
                                    mediaPreview = `<iframe src="${newFileUrl}" class="card-img-top" style="height: 200px;" frameborder="0" title="${fileTitle}"></iframe>`;
                                } else if (['text'].includes(fileType)) {
                                    mediaPreview = `
                            <div class="d-flex align-items-center justify-content-center" style="height: 200px; background-color: #f8f9fa;">
                                <a href="${newFileUrl}" target="_blank" class="text-center">
                                    <img src="${currentUrl}assets/images/text.png" alt="${fileType}" style="height: 200px;">                                    
                                </a>
                            </div>`;
                                } else if (fileType === 'apk') {
                                    mediaPreview = `
                            <div class="d-flex align-items-center justify-content-center" style="height: 200px; background-color: #f8f9fa;">
                                <a href="${newFileUrl}" download class="text-center">
                                    <img src="${currentUrl}assets/images/android.avif" alt="APK" style="height: 200px;">
                                </a>
                            </div>`;
                                } else {
                                    mediaPreview = `
                            <img src="${currentUrl}assets/images/default-thumbnail.png" class="card-img-top img-fluid" alt="${fileTitle}" style="object-fit: contain; height: 200px;">`;
                                }
                        ...
                        <div class="card h-100 w-100">
                            ...
                            <div class="card-body d-flex flex-column">
                                <h5 class="card-title">${fileTitle}</h5>
                                <p class="card-text flex-grow-1">${project.media.media_description}</p>
                                <p class="card-text flex-grow-1">Type: ${fileType.toUpperCase()} | Size: ${fileSize} Kb</p>
                                <div class="mt-auto d-flex flex-column flex-sm-row gap-2">
                                    <button type="button" class="btn btn-primary flex-fill" data-bs-toggle="modal" data-bs-target="#mediaModal"
                                        onclick="showMedia('${fileTitle}', '${fileType}', '${project.media.file_url}')">
                                        View ${fileType.toUpperCase()}
                                    </button>
                                    <a href="${newFileUrl}" class="btn btn-secondary flex-fill" download>
                                        Download
                                    </a>
                                </div>
                            </div>
                    ...
                    `;
                            ...
                            resultDiv.innerHTML = '<p class="text-center text-danger w-100">No media found.</p>';
        ...
        function showMedia(title, fileType, fileUrl) {
            ...
            // console.log('showModel', fileType);
